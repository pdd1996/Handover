...
| GET `/records/{id}` | 登录用户 | 交接单详情 | F2-03、F5-01 | 含全部读数、标红项（alerts，**置顶序** level high→mid→low 由服务端排好，shared `ALERT_LEVEL_RANK`）、电梯核对（联字典回显电梯名）、双方确认信息、历史版本摘要 `versions`（F2-07-T1「历史版本可查」：version/修改人/时刻/变更字段旧值新值，全字段快照留 record_versions 不入响应，TK-20 起）；含科长批注 `chief_note`（覆盖式当前值，TK-24）；权威形状 shared dto `RecordDetailDto`（TK-18 落地，接班人浏览与科长巡查共用） |
...
| 方法与路径 | 角色 | 用途 | 关联规格 | 契约要点 |
| --- | --- | --- | --- | --- |
| GET `/records` | 登录用户 | 历史记录筛选 | F5-01、F6-01 | 筛选参数 `from/to/submitter_id/status` **全部可选取交集**（from/to 须合法日历日、submitter_id 正整数、status 四状态枚举；非法取值 400 逐条点名——解析单一实现 parseRecordListFilters，与 GET /admin/records/export 共用，TK-24）；响应权威形状 shared dto `RecordListDto`——`items[]` 按 duty_date **倒序**（id/record_no/duty_date/status/version/submitted_at/confirmed_at/submitter/receiver/alert_count/chief_note，标红项数与详情同源）；**不分页**（班次一天一条、日增 1 行，与 pending/notifications 同口径）；角色列「登录用户」= SessionGuard + `@Roles('master','chief')`（同 GET /records/{id} 口径），师傅看全部、科长同；F6-01 科长半边（记录管理页）随 TK-24 落地，F5-01 师傅端历史查询界面属 P2/TK-35、接口先行共用同一取数 |
...
| 方法与路径 | 角色 | 用途 | 关联规格 | 契约要点 |
| --- | --- | --- | --- | --- |
| GET/POST `/admin/users`，PATCH `/admin/users/{id}` | chief | 账号开通/停用 | F6-02 | **GET**：全量账号 **id 升序**（= 开通顺序，含科长行——只读展示，启停仅对师傅账号开放，D-T25 ②）；权威形状 shared dto `UserListDto`（items: id/username/real_name/role/status/created_at；passwordHash 凭证不出网）。**POST**：开通**师傅账号**——角色恒 master（科长账号经部署初始化发放，不走本接口，D-T25 ①），body `{username, real_name, password}`（权威形状 shared dto `UserCreatePayloadDto`）：username 1~32 位字母/数字/下划线（UNIQUE，重复 400 点名 username）、real_name 1~32 字、初始密码 8~64 字（bcrypt 加盐哈希落库、明文不回显不出网），违规 400 逐条点名。**PATCH**：body `{status: active \| disabled}`（权威形状 shared dto `UserStatusPatchPayloadDto`，Phase 1 账号管理动作仅状态启停一项，D-T25 ②）——**停用即不可登录**：状态翻转与审计同事务提交后删除该用户全部 `sessions` 存根（D-T13），已在线设备下一次请求即 401；chief 目标 403 FORBIDDEN（F6-02 范围 = 师傅账号）；**值无变化不更新不写审计**（D-T21 M1 同一精神）；未知/非数字 id 404（同 GET /records/{id} 口径）（TK-25 落地，决策记录 D-T25） |
| GET/PUT `/admin/schedules` | chief | 排班月视图维护 | F6-03、F6-04 | **GET**：月视图，`?month=YYYY-MM` 缺省当前墙钟月（SHIFT_TIMEZONE 本地日历），**稀疏列示**该月排班行（无排班日不出行）、duty_date 升序，姓名联 users 回显（停用账号的历史排班仍显原名）；非法 month 400 点名；权威形状 shared dto `ScheduleMonthDto`（items: duty_date/user_id/real_name/updated_at）。**PUT**：单日单条维护（body `{duty_date, user_id}`，权威形状 shared dto `SchedulePutPayloadDto`）——一天一人由 schedules.duty_date UNIQUE 承载，同日已有排班即**改派**（INSERT…ON DUPLICATE KEY UPDATE，无 1062 面）；duty_date 须日历有效（isValidCalendarDate 单一权威，拦 02-30）、user_id 须为**存在的师傅账号**（chief 目标与未知 id 均 400 点名 user_id——排班人恒为师傅，C-05；停用账号不新增指派但历史行保留）；**同值重复 PUT 值无变化不更新不写审计**（D-T21 M1 同一精神）；变更与审计 `schedule.update` 同事务（§5），响应 `{duty_date, user_id, real_name, changed, updated_at}`（权威形状 shared dto `SchedulePutResultDto`，changed=false 即未变更）。落库即驱动接班人带出（F2-01）与漏交检测（F6-06）——消费方读同一张 schedules 表，改派次班生效（TK-26 落地，决策记录 D-T26） |
...
| GET `/admin/records/export` | chief | 记录导出 | F6-01 | Phase 1 交付 **CSV 附件**（UTF-8 带 BOM + CRLF，Excel 直开不乱码；`Content-Disposition: attachment`，文件名按筛选区间 `records-YYYY-MM.csv`（同月）/ `records-{from}_{to}.csv`（跨月）/ `records-{导出当日}.csv`（无筛选））；筛选参数与 §3.5 GET /records **完全同形**（解析单一实现）；Phase 1 列集钉死（订正 28）：交接单号 / 班次日期 / 状态（中文四值）/ 版本 / 交班人 / 接班人 / 提交时刻 / 确认时刻 / 水用量 / 电用量 / 天然气用量 / 液氧日用量 / 标红项数 / 科长批注；「按月」为后台界面的常用区间（记录管理页默认近 30 天），接口层接受任意日历区间；xlsx 与正式月报（每日用量明细 + 异常汇总）挂 P2 升级（F5-04 ⏸）（TK-24 落地） |
| POST `/admin/records/{id}/annotation` | chief | 科长批注 | F6-01 | 覆盖式单条备注（records 新列 `chief_note` varchar(500)，技术方案修订 30）：body `{note}`，trim 后**空串 = 清除**（置 NULL）、非空 = 覆盖写入，上限 500 字（越界 400 点名 `chief_note`）；**不限记录状态**（draft 撤回单上批「请尽快重提」是合法管理动作，D-T24）；**值无变化不更新不写审计**（D-T21 M1 数值判等同一精神）；写入/清除/历次修改均审计 `record.annotate`（§5）＝「批注留痕」判据；不进 record_versions（非交接单数据变更，version 不动）；未知/非数字 id 同 GET /records/{id} 口径 404；权威形状 shared dto `AnnotationResultDto`（TK-24 落地，决策记录 D-T24） |
| GET `/admin/missing-submits` | chief | 应提交未提交视图 | F6-06 | 日期 + 排班人（**duty_date 倒序**，最近漏交在前）；数据源与 missing_submit 通知**同源**（漏交判定单一实现：截止在墙钟空间、records 行存在即不算漏交（台账增补 #27）、窗口 = 当前班次 − `backfill_window_days`，D-T21 L3）；权威形状 shared dto `MissingSubmitListDto`（items: duty_date / user_id / real_name）（TK-23 落地） |
...
| POST `/admin/records/{id}/annotation`（科长批注，TK-24） | `record.annotate` | 覆盖式单条批注留痕（D-T24）：`old_value`/`new_value` 携 `{note}` 前后值（清除时 new_value.note=null）；**值无变化不更新不写审计**（D-T21 M1 同一精神）；批注不进 record_versions（非交接单数据变更，version 不动） |
| POST `/admin/users`（开通，TK-25）、PATCH `/admin/users/{id}`（启停） | `user.update` | 新旧值（开通 oldValue=null、newValue 搭 `{username, real_name, role, status}` 开通值；启停携 status 前后值；值无变化不写审计） |
| PUT `/admin/schedules`（单日排班维护，TK-26） | `schedule.update` | 新旧值：oldValue/newValue 携 `{user_id, real_name}` 前后值（首次指派 oldValue=null）、targetId=duty_date；操作人/时刻即 actor_id/created_at；**同值重复 PUT 值无变化不写审计**（D-T21 M1 同一精神） |
| `/admin/elevators`… | `elevator.update` | 新旧值 |
| `/admin/spots`… | `spot.update` | 新旧值 |
| PUT `/admin/configs/{key}` | `config.update` | 新旧值 |
| POST `/records/today/submit`、`/records/backfill`（排班安全阀确认放行，TK-26/F6-05） | `record.guard_confirm` | 仅**命中**（登录提交人 ≠ 该班次排班人且经 `duty_guard_confirm.confirmed=true` 放行）的本次提交写入**一行**：oldValue 记排班基线 `{user_id, real_name}`、newValue 记实际提交人 `{user_id, real_name, version}`、reason 记调班原因（**选填**，demo 口径确认即放行留痕）；排班一致时上送的确认被忽略不写审计（同「未命中确认不入账」口径） |
...
27. **v0.1 订正（2026-09-16）**：TK-23（后台框架与权限，M4 起步）落地联动——① **§3.6 GET `/admin/missing-submits` 行要点回填**（原文仅一句纲要）：duty_date 倒序、数据源与 missing_submit 通知同源（漏交判定单一实现 NotificationsService.missingSubmitShifts：截止在墙钟空间、records 行存在即不算漏交（增补 #27）、窗口 = 当前班次 − `backfill_window_days`，D-T21 L3）、权威形状 shared dto `MissingSubmitListDto`（items: duty_date/user_id/real_name）。② **§3.6 守卫框架口径（本条留痕，适用于本节全部路由）**：科长后台路由一律**类级** `@UseGuards(SessionGuard, RolesGuard)` + `@Roles('chief')`——未登录 401 UNAUTHENTICATED、登录但角色不足（master 访问）403 FORBIDDEN；后续 TK-24~29 新挂 /admin 路由沿用同一类级三行，admin.spec 的权限矩阵（master → 全部 admin 路由 403）与路由完备性哨兵（已注册 /admin 路由逐字对账契约清单）随路由表增长自动执法。错误码表 13 项不变、路由总数 34 不变（missing-submits 系 §3.6 既有行）。测试：apps/api admin.spec 8 项（权限矩阵 master/未登录/chief 三向 + 视图黄金值 D+1~D+7 倒序、#27 视图口径、窗口外不展示、路由层冒烟），E2E admin-shell.spec 3 项（desktop-admin 新项目：chief 布局 / master 无权限页 / 未登录回登录页）。联动：台账增补 #37、技术方案修订 29、任务分解修订 40。
28. **v0.1 订正（2026-09-17）**：TK-24（记录管理：查看/筛选/导出/批注）落地联动——① **§3.5 GET `/records` 行要点回填**（原文仅一句纲要）：四参数可选取交集、非法 400 逐条点名（解析单一实现 parseRecordListFilters，与导出共用）、权威形状 shared dto `RecordListDto`（duty_date 倒序、含 alert_count 与 chief_note）、不分页（班次一天一条）、角色口径同 GET /records/{id}；F6-01 科长半边随 TK-24 落地、F5-01 师傅端界面属 P2/TK-35 接口先行。② **§3.6 导出行交付形态钉死**：Phase 1 = CSV 附件（UTF-8 带 BOM + CRLF、文件名按筛选区间、14 列列集逐字列明），筛选参数与 §3.5 同形；「按月」为界面常用区间、接口接受任意区间；xlsx/正式月报挂 P2 F5-04 ⏸。③ **§3.6 新增路由 POST `/admin/records/{id}/annotation`（科长批注）**——PRD §6.6/台账 F6-01/用例 F6-01-T1 皆含「批注」，契约此前无对应路由，随 TK-24 补齐（覆盖式单条 `chief_note`、空串=清除、500 字上限、不限记录状态、值无变化不写审计、审计 `record.annotate`、不进 record_versions；决策记录 **D-T24** 拍板）。④ **§3.4 GET /records/{id} 行补 `chief_note`**（批注当前值随详情出网）。⑤ **§5 审计联动表新增 `record.annotate` 行**。错误码表 13 项不变（400/404 复用既有码）；**路由总数 34 → 35**（批注为新路由；export 与 /records 均系既有行）。测试：apps/api admin-records.spec **15 项**（F6-01-T1：筛选黄金值/参数点名/导出 CSV 附件/批注留痕三处回读/同值不写审计/清除/draft 可批/404）+ admin.spec 哨兵扩至 3 条路由；jest 全量 **243/243**（重灌种子）、E2E desktop-admin **6/6**（admin-records 3 项：列表筛选详情、批注真链路写入回读清除、导出下载）。联动：台账增补 #38、技术方案修订 30、决策记录 D-T24（增补 19）、任务分解修订 41。
29. **v0.1 订正（2026-09-17）**：TK-25（人员管理：账号开通/停用）落地联动——① **§3.6 users 行要点回填**（原文仅一句纲要）：GET 全量账号 id 升序（含科长行只读展示、启停仅对师傅开放）、POST 开通师傅账号（角色恒 master，username/real_name/password 校验 400 逐条点名、重复名 400 点名、bcrypt 落库）、PATCH 状态启停（停用与审计同事务、提交后删该用户全部 sessions 存根——停用即不可登录，D-T13；chief 目标 403；值无变化不写审计；未知/非数字 id 404），权威形状 shared dto `UserListDto`/`UserCreatePayloadDto`/`UserStatusPatchPayloadDto`（决策记录 **D-T25** 拍板）。② **§5 审计联动表 `user.update` 行精确化**：触发语境由「PUT /admin/users…」订正为实际路由「POST /admin/users（开通）、PATCH /admin/users/{id}（启停）」，补新旧值口径（开通 oldValue=null；启停携 status 前后值；值无变化不写审计）。错误码表 13 项不变（400/403/404 复用既有码）；**路由总数 35 → 38**（GET/POST/PATCH users 三条均系 §3.6 既有行拆落地）。测试：apps/api admin-users.spec **10 项**（F6-02-T1：查询黄金值从库反查、开通审计操作人/时刻、参数与重复名点名、停用即删存根 + 停用后登录拒绝联动 F1-11-T3、启用恢复、同值不写审计、chief 403/404/越值 400）+ admin.spec 哨兵扩至 6 条路由；jest 全量 **253/253**（重灌种子）、E2E desktop-admin **9/9**（admin-users 3 项：账号全景渲染、开通真链路、停用→启用真链路）。联动：台账增补 #39、技术方案修订 31、决策记录 D-T25（增补 20）、任务分解修订 42。
30. **v0.1 订正（2026-09-17）**：TK-26（排班管理与安全阀）落地联动——① **§3.6 schedules 行要点回填**（原文仅一句纲要）：GET 月视图（?month 缺省当前墙钟月、稀疏列示、duty_date 升序）、PUT 单日单条 upsert（一天一人 duty_date UNIQUE；日历校验与师傅账号校验 400 逐条点名；同值不写审计；改派即生效驱动带出与漏交检测），权威形状 shared dto `ScheduleMonthDto`/`SchedulePutPayloadDto`/`SchedulePutResultDto`（决策记录 **D-T26** 拍板）。② **§3.2 submit 行与 §4 提交协议**：排班安全阀 409 DUTY_MISMATCH 落地——判定置于防呆 409 之前（身份对账先于业务防呆），`duty_guard_confirm.confirmed === true` 为唯一解锁（拒绝/未确认/畸形一律再 409）、reason 选填、排班缺失不判定、排班一致时确认被忽略；**补交链路同门**（订正 18 挂账「补交班次的排班归属校验」就此闭环，防师傅冒名补交他人班次）。③ **§5 审计联动表两行精确化**：`schedule.update` 行补触发语境与新旧值口径；`record.guard_confirm` 行补触发语境（submit + backfill）、仅命中写一行、oldValue/newValue/reason 形状。错误码表 13 项不变；**路由总数 38 → 40**（GET/PUT /admin/schedules 系 §3.6 既有行拆落地）。测试：apps/api admin-schedules.spec **8 项**（F6-03-T1：月视图黄金值/非法 month/改派审计新旧值与操作人/同值不写审计/新增日 oldValue=null/参数违法点名 + F6-04-T1：提交带出与排班一致/改派后带出生效/漏交检测数据可查）+ records-duty-guard.spec **7 项**（F6-05-T1：409 点名与文案、确认放行落库 + 审计三要素、排班人本人不触发、一致时确认被忽略；F6-05-T2：三种拒绝形态不落库、排班缺失不判定、补交归属校验）+ admin.spec 哨兵扩至 8 条路由；jest 全量 **268/268**（重灌种子）、E2E 全量 **66/66**（E2E_CHANNEL=chrome）。联动：台账增补 #40、技术方案修订 32、决策记录 D-T26（增补 21）、任务分解修订 43、《开发种子数据》修订 7（种子轮值相位锚定 D0=zhang）。
