 ...
 * 后续按任务队列扩充本模块路由：TK-12 预览与提交（/records/today/preview、/submit）、
 * TK-20 异议修改重提（/records/{id}/resubmit）、TK-21 撤回（/records/today/withdraw）。
 * （TK-07 上一班带出 /records/today/prev 已落地；在线草稿不设服务端端点——草稿层在客户端
 * IndexedDB，决策记录 D-T18，契约 §3.2 GET/PUT /records/today/draft 两行已订正为暂缓。）
 */
@Module({
  imports: [AuthModule],
